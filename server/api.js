...
router.patch("/bookings/:id/toggle-voting", async (req, res) => {
  try {
    const booking = await prisma.booking.findUnique({
      where: { id: req.params.id },
    });
...
router.patch("/bookings/:id/toggle-2bb", async (req, res) => {
  try {
    const booking = await prisma.booking.findUnique({
      where: { id: req.params.id },
      include: { twoBallTeams: true },
    });
    ...
    const newIs2BB = !booking.is2BB;

    if (newIs2BB && booking.twoBallTeams.length === 0) {
      ...
      const participantsWithHandicap = parsedParticipants
        .map((p) => {
          const member = allMembers.find((m) => m.phone === p.phone);
          return {
            phone: p.phone,
            name: p.nickname || p.name,
            handicap: member ? parseFloat(member.handicap) || 36 : 36,
          };
        })
        .sort((a, b) => a.handicap - b.handicap);

      const teams = [];
      const count = participantsWithHandicap.length;
      const half = Math.floor(count / 2);

      for (let i = 0; i < half; i++) {
        const lowPlayer = participantsWithHandicap[i];
        const highPlayer = participantsWithHandicap[count - 1 - i];
        teams.push({
          bookingId: req.params.id,
          player1: lowPlayer.phone,
          player2: highPlayer.phone,
        });
      }

      if (count % 2 === 1) {
        const middlePlayer = participantsWithHandicap[half];
        teams.push({
          bookingId: req.params.id,
          player1: middlePlayer.phone,
          player2: middlePlayer.phone,
        });
      }

      if (teams.length > 0) {
        await prisma.twoBallTeam.createMany({
          data: teams,
          skipDuplicates: true,
        });
      }
    ...
    const updated = await prisma.booking.update({
      where: { id: req.params.id },
      data: { is2BB: newIs2BB },
      include: { organizer: true, twoBallTeams: true },
    });

    req.io.emit("bookings:updated");
    res.json(updated);
  } catch (error) {
    console.error("Error toggling 2BB:", error);
    res.status(500).json({ error: "Failed to toggle 2BB" });
  }
});

router.get("/bookings/:id/2bb-teams", async (req, res) => {
  try {
    const teams = await prisma.twoBallTeam.findMany({
      where: { bookingId: req.params.id },
    });
    res.json(teams);
  } catch (error) {
    console.error("Error fetching 2BB teams:", error);
    res.status(500).json({ error: "Failed to fetch 2BB teams" });
  }
});

router.put("/bookings/:id/2bb-teams", async (req, res) => {
  try {
    const { teams } = req.body;

    await prisma.twoBallTeam.deleteMany({
      where: { bookingId: req.params.id },
    });
    ...
    const updatedTeams = await prisma.twoBallTeam.findMany({
      where: { bookingId: req.params.id },
    });
    ...
    console.error("Error updating 2BB teams:", error);
    res.status(500).json({ error: "Failed to update 2BB teams" });
  }
});
