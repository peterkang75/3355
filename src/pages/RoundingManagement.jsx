...
function RoundingManagement() {
  ...
  const handleToggle2BB = async () => {
    ...
    try {
      await apiService.toggle2BB(bookingId);
      await refreshBookings();
    } catch (error) {
      alert('2BB 상태 변경에 실패했습니다.');
    } finally {
        ...
        <div className="card" style={{ marginBottom: '16px', padding: 0, overflow: 'hidden' }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              <div>
                <div style={{ fontSize: '15px', fontWeight: '600', color: '#1a1a1a' }}>2BB (팀전)</div>
                <div style={{ fontSize: '13px', color: '#888', marginTop: '2px' }}>
                  {booking.is2BB ? '2인 베스트볼 활성화' : 'OFF'}
                </div>
              </div>
            </div>
            <button
              ...
              style={{
                ...
                borderRadius: '15px',
                border: 'none',
                background: booking.is2BB ? '#9333ea' : '#ddd',
