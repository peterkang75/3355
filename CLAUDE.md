...
7. **단계별 실행 규칙** — 플랜 승인 후 한번에 전부 구현하지 않는다
   - 플랜 완성 시 작업을 효율적인 단위(Step)로 나눠서 사장님께 제시
   - 각 Step을 순서대로 하나씩 실행하고, Step 완료마다 `npx vite build`로 빌드 확인
   - 빌드 성공 확인 후 다음 Step으로 진행
   - 빌드 실패 시 해당 Step 내에서 즉시 수정 후 재빌드
