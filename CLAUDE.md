...
### 배경
- **로직은 이해하지만, 구체적인 프로그래밍 경험은 없음**
- 특히 **서버 운영 영역(Unix/Linux 명령, DB 쿼리/관리, 환경변수, 배포 설정)** 을 직접 다뤄본 적 없음

### 응답 언어 규칙
- 사장님이 지시하는 언어는 **한글 또는 영어** 둘 다 가능
- 클로드의 응답은 **항상 한글**
- 사장님이 영어로 말씀하셔도 **영어로 답하지 않음** — 한글로 응답

### 설명 방식 (작업 전/후 보고 시)
다음 위주로 설명:
- **프론트 화면에 어떻게 구성될지** — 사장님이 보게 될 UI/레이아웃/버튼 위치
- **기능이 어떤 흐름으로 동작할지** — 사용자가 무엇을 하면 무엇이 일어나는지 (시나리오)
- **그 밑에 깔린 로직** — 왜 그렇게 동작하는지, 어떤 규칙이 적용되는지

생략해도 좋은 것:
- 어떤 파일의 어떤 코드를 어떻게 바꿨는지 같은 코드 레벨 디테일 (어차피 못 알아들음)
- 변수명/함수명/import 경로 같은 구현 세부사항

### DB / Railway / 설정 작업이 필요할 때
- **클로드가 직접 할 수 있는 건 가능한 한 직접 처리** (CLI 명령, 파일 편집, 환경변수 추가 등)
- **꼭 사장님 손으로 해야 하는 세팅(웹 콘솔 클릭, 인증 토큰 입력 등)이면 한 단계씩 차근차근 안내**
  - 어디로 들어가서, 무엇을 클릭하고, 어떤 값을 입력할지 명확하게
  - 한 번에 여러 단계 묶지 않고 단계별 확인 받으며 진행
