...
- [ ] **Step 4: 시드 — 회원/라운딩/미디어/자유글/스코어 (운영 DB 접근 0)**

> **운영 DB는 읽지도 않는다** (자동 안전장치가 운영 조회를 차단함). 모든 시드는 로컬에서 자체 생성. 사진은 R2 없이도 UI가 깨지지 않게 처리.

일회성 시드 스크립트 `server/scripts/seed-local-feed.js` (오직 `LOCAL_DB_URL`만 사용):
- **회원:** 정회원 2명, 게스트 1명(`isGuest:true, approvalStatus:'guest'`), 운영자 1명(`role:'관리자'`). 모두 `isActive:true, approvalStatus:'approved'`(게스트 제외). **phone 값 메모해 둘 것**(curl 테스트용).
- **라운딩:** 지난 booking 2건(`type:'정기모임'`, `date` 과거). `participants`는 **위 정회원들의 phone을 포함**한 JSON 문자열 배열 (예: `[JSON.stringify({name:'홍길동',phone:'01011112222'}), ...]`) — 미디어 업로드 권한 테스트가 통과하도록.
- **라운딩 미디어:** 각 booking에 `RoundingMedia` 2~3행, `status:'ready'`, `objectKey:'bookings/seed/N.jpg'`, **`thumbnailKey:null`**. → 피드 cover는 `thumbnailKey`가 null이라 `coverThumbUrl:null` → RoundPostCard가 **깔끔한 회색 박스 + 📷N 뱃지**로 표시(깨진 이미지 없음). mediaCount로 게시물 생성 조건(ready≥1) 충족.
- **자유글(FeedPost):** ① 텍스트만, ② 텍스트+유튜브 링크(예: `https://youtu.be/dQw4w9WgXcQ` — 썸네일이 img.youtube.com에서 실제 로드되어 임베드 검증 가능), ③ 텍스트+인스타 링크(`https://instagram.com/p/xxx` — 링크카드 검증). 미디어 행 없이.
- **스코어:** 한 booking에 대해 `Score` 몇 건(리더보드 링크 확인용, `roundingName`=그 booking의 title).

실행:
```bash
LOCAL_DB_URL="$LOCAL_DB_URL" node server/scripts/seed-local-feed.js
```
Expected: 로컬 시드 완료 로그 + 출력에 시드된 회원 id/phone(정회원·게스트·운영자) 표시. **운영 DB·R2 접근 0.**
...
**백엔드**
- 생성: `server/utils/feedMedia.js` — 자유글 미디어 처리(압축·썸네일·R2). **기존 `utils/media.js`의 `processImage`/`processVideo` 프리미티브를 재사용**, `feedPostMedia` 행 갱신. (운영 중인 `media.js`는 **무수정** — 회귀 위험 0)
- 생성: `server/routes/feed.js` — 피드 목록 / 자유글 CRUD / 자유글 미디어 / 반응 / 댓글 라우트
...
## Task 2: 자유글 미디어 처리 헬퍼 (feedMedia.js — media.js 무수정)

**핵심 결정(안전):** 무거운 압축 로직 `processImage`/`processVideo`는 이미 `server/utils/media.js`에 공용으로 분리돼 있다(`media.js`가 거기서 import). 따라서 운영 중인 `server/routes/media.js`는 **건드리지 않고**, 자유글용 작은 처리 함수만 새로 만들어 같은 프리미티브를 재사용한다. (기존 라운딩 업로드 회귀 위험 0)

참고 — 기존 `media.js`의 background 처리 형태(그대로 미러링):
```javascript
const { processImage, processVideo } = require('../utils/media');
// job = { rowId, baseKey, objectKey, isVideo, path }
// processed = await processImage(path)  →  { fullBuf, contentType, thumbBuf?, durationSec?, width?, height? }
// r2.uploadBuffer(objectKey, processed.fullBuf, processed.contentType)
// thumbnailKey = processed.thumbBuf ? `${baseKey}-thumb.jpg` : null
```

**Files:**
- Create: `server/utils/feedMedia.js`

- [ ] **Step 1: feedMedia.js 작성**
...
// jobs: [{ rowId, baseKey, objectKey, isVideo, path }], updateRow(id, data)
...
Run: `npx vite build` → 성공 (문법 확인). (런타임 검증은 Task 4에서 자유글 업로드와 함께.)
```bash
git add server/utils/feedMedia.js
git commit -m "feat(feed): 자유글 미디어 처리 헬퍼(feedMedia) — utils/media 재사용"
```
...
const upload = multer({ dest: os.tmpdir(), limits: { fileSize: 200 * 1024 * 1024 } }); // media.js와 동일 설정
```
...
router.post('/posts/:id/media', requireAuth, upload.array('files', 10), async (req, res) => {
  ...
  processFeedMediaJobs(jobs,
    (id, data) => prisma.feedPostMedia.update({ where: { id }, data }))
    .catch((e) => console.error('feed media process error', e));
});
```

> job 객체 필드(`{ rowId, baseKey, objectKey, isVideo, path }`)는 `media.js`의 기존 형태와 동일 — `feedMedia.processFeedMediaJobs`가 그대로 소비한다.
...
import { useState } from 'react';
import apiService from '../../services/api';
import { compressImageFile } from '../../utils/compressImage'; // MediaGallery와 동일 경로
    ...
    try {
      ...
      if (files.length > 0) {
        const imgs = await Promise.all(files.map((f) =>
          f.type.startsWith('image/') ? compressImageFile(f) : f)); // compressImageFile(file) — 1인자, File 반환
...
export default function Feed() {
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '12px 12px 90px' }}>
      ...
      {galleryBooking && (
        <MediaGallery booking={galleryBooking} user={user} onClose={() => { setGalleryBooking(null); load(); }} />
      )}
    </div>
  );
}
...
> `MediaGallery` 계약(확인됨): `export default function MediaGallery({ booking, user, onClose })`, import `from './booking/MediaGallery'`. `user` prop 필수(업로드 권한·삭제 판단에 사용) — 반드시 전달.
    ...
    {pastRegular.length === 0 ? (
      ...
      return (
        <div key={b.id} style={{ background: '#fff', borderRadius: 12, padding: '14px 16px',
          ...
          <button onClick={() => navigate(`/leaderboard?id=${b.id}`)}
...
`mediaPreviews` 관련 state·useEffect(~138-146)와 import가 '지난' 탭에서만 쓰였다면 제거. (다른 곳에서 안 쓰면 깔끔히 삭제.) "스코어·시상 보기" 링크는 **확인된 형식 `/leaderboard?id=${b.id}`** 사용 — `Leaderboard.jsx`가 `searchParams.get('id')`로 booking을 찾는다(검증됨).
...
**확인 완료 (코드 조각 확보, 본문에 반영):**
- 미디어 job 객체 형태 `{ rowId, baseKey, objectKey, isVideo, path }` + `processImage/processVideo`(`utils/media`) 반환 `{ fullBuf, contentType, thumbBuf?, durationSec?, width?, height? }` — media.js에서 확인. `feedMedia.js`가 동일 형태 사용. media.js 무수정.
- `compressImageFile`: `src/utils/compressImage.js`, **1인자** `compressImageFile(file)`, File 반환. import `'../../utils/compressImage'`.
- `MediaGallery`: `({ booking, user, onClose })`, import `'./booking/MediaGallery'`. user 필수.
- 리더보드: `/leaderboard?id=${bookingId}` (Leaderboard.jsx가 `searchParams.get('id')`로 조회).
- 미디어 목록 조회 라우트는 이미 `media.js`에 존재(`GET /api/bookings/:id/media`, 서명 URL 포함) — MediaGallery가 그대로 사용하므로 라운딩 갤러리는 추가 작업 불필요.

**남은 런타임 의존(서브에이전트 검증 필수):**
- `feedMedia` 런타임 동작(R2 업로드)은 자동 테스트 제외(운영 R2) → Task 10 사장님 로컬 dev에서 실사진 1장으로 확인.
