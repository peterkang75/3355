...
Run: `npm run db:push` (= `prisma migrate dev`)
마이그레이션 이름 입력 프롬프트가 뜨면 `add_feed_models` 입력.
Expected: 4개 테이블 생성 + `@prisma/client` 재생성, 에러 없음.
...
## Task 10: 통합 점검 + Plan.md 갱신

- [ ] **Step 1: 전체 시나리오 수동 검증 (npm run dev)**

- 정회원: 소식 진입 → 라운딩 사진 게시물 + 자유글 혼합·최신순 → ♡/댓글 → 자유글 작성(글+사진, 유튜브 링크 임베드 확인, 인스타 링크 카드 확인) → 본인 글 삭제.
- 새 사진을 어떤 지난 라운딩에 올리면(갤러리에서) 그 라운딩 게시물이 피드 최상단으로 올라오는지.
- 운영자: 남의 댓글/자유글 삭제 가능.
- 게스트: 보기·♡ 가능, 댓글·작성 불가(입력창·+ 버튼 숨김 + 서버 401).
- 네비 5개 유지, 더보기에 관리(운영자만).
- 라운딩 '지난' = 사진 없는 정보/스코어.
...
`Plan.md` "현재 상태" 표에 `| 9 | 소식 커뮤니티 피드 | ✅ 완료 (YYYY-MM-DD) |` 추가, 우선순위 문구 갱신, Phase 9 상세 섹션 추가(구현 메모: 공용 Reaction/Comment 다형성 테이블, 자유글은 정회원 전용, 게스트 반응만, 유튜브 임베드/링크카드, 라운딩 지난 탭 정보전용 전환).
...
git push
```
(푸시 시 Railway 자동 배포 — 배포 후 프로덕션에서 1) DB 마이그레이션 적용 확인 2) 소식 탭 동작 확인.)
